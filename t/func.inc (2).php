...
function get_tm($val) {
			...
			if( date("d",$tm)==$d && date("m",$tm)==$m && date("Y",$tm)==$y && date("H",$tm)==$h && date("i",$tm)==$i) {
				...
				if($tm>time()) {
					...
					if(date("d.m.Y")==date("d.m.Y",$tm)) {
						...
						$now=date("d.m.Y H:i");
						print "<p class='red'>Ошибка: Вы указали дату в прошедшем времени. Сейчас - <b>$now</b>, указано <b>$val</b><br></p>";
						return $tm;
					}
				}
			}
	...
	print "<p class='red'>Ошибка: Неправильный формат даты.: <b>$val</b></p>";
	return false;
}
function save_prior($id, $prior) {
	mysql_query("UPDATE tasks SET prior=$prior WHERE id=$id");
}
function get_top() {
...
function get_up($id, $num) {
	$res=mysql_query("SELECT prior FROM tasks WHERE id=$id");
	if(!$res) return false;
	$r=mysql_fetch_row($res);
	$prior=$r[0];
	$res=mysql_query("SELECT prior FROM tasks WHERE del=0 AND prior>0 AND prior>$prior ORDER BY prior ASC");
	if(!$res) return false;
	if(mysql_num_rows($res)>$num) {
		for($i=0; $i<$num; $i++) {
			$r=mysql_fetch_row($res); $p1=$r[0];
		}
		$r=mysql_fetch_row($res); $p2=$r[0];
		$p=($p1+$p2)/2;
...
function get_down($id, $num) {
	...
	$r=mysql_fetch_row($res);
	$prior=$r[0];
	$res=mysql_query("SELECT prior FROM tasks WHERE del=0 AND prior>0 AND prior<$prior ORDER BY prior DESC");
	if(!$res) return false;
	if(mysql_num_rows($res)>$num) {
		for($i=0; $i<$num; $i++) {
			$r=mysql_fetch_row($res); $p1=$r[0];
