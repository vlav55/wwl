...
function api_request($endpoint,$data,$method) {
...
$res=api_request('/partner/',$data,'GET');
if(!isset($res['uid'])) {
	die("Error login");
	exit;
}
    ...
    <section class="service" id='top'>
      <div class="container">
        <div class="possibilities">
			...
			<? } ?>
			<? if(!isset($lk_no_disp_cashout) || !$lk_no_disp_cashout) {?>
				...
				<div class='' id='details'>  
					<form method='POST' action='?uid=<?=$uid_md5?>&u=<?=$_GET['u']?>'>
						<div>
						 ...
						 <button class='button1 m-1' name='do_details' value='yes'>Сохранить</button>
						 <button class='button1 bg-primary m-1'  data-toggle="modal" data-target="#cashoutModal" onclick="event.preventDefault();">Вывести средства</button>
						 <? //$db->notify_me("HERE_ $insales_id"); ?>
						 <?if($insales_id && (isset($insales_lk_cashout_button) ? $insales_lk_cashout_button : true) ) { ?>
							<button class='button1 bg-success m-1'  data-toggle="modal" data-target="#cashoutModal_insales" onclick="event.preventDefault();">Вывести бонусами магазина</button>
						 <?} ?>
						</div>
					</form>
				</div>
			...
			<div>
				...
				<div class='<?=$collapse?>' id='partner_fee'>
					...
					<table class='table table-striped table-responsive'  style="margin: 0 auto; display: inline-table;">
						<thead>
							<tr>
								<th>Наименование</th>
								<th>Цена</th>
...
-->
			...
			<div>
				...
				if($ctrl_id==1) {
					...
					//$db->notify_me("HERE s=$suf bc=$bc ".print_r($m,true));
			...
			$res=$db->query("SELECT promocode, MIN(product_id) AS product_id,COUNT(product_id) AS cnt_pid, cnt,tm2,price,discount,fee_1,fee_2
			...
			<div  class='card_ py-3 <?=$collapse?>' id='partner_promocodes'>
				<div class='table-responsive' >
				<table class="table table-responsive"  style="margin: 0 auto; display: inline-table;">
					<thead>
						<tr>
							<th>Промокод</th>
							<th>Действует по</th>
					...
					<tbody>
						...
						while ($r = $db->fetch_assoc($res)) {
							...
							$descr.=($r['cnt_pid']>1) ? " <br>... и еще ".($r['cnt_pid']-1)." продукта(ов)" :"";
							$price = htmlspecialchars($r['price']);
							$discount = htmlspecialchars($r['discount']);
							$price = !$r['discount'] ? $price."р." : "-";
			...
			<div class=''>
				...
				<table class='table'  style="margin: 0 auto; display: inline-table;">
					...
					<tbody>
						...
						<tr><td>Сумма оплат</td><td><?=formatNumber($sum_buy_today)?></td><td><?=formatNumber($sum_buy_yesterday)?></td><td><?=formatNumber($sum_buy_this_week)?></td><td><?=formatNumber($sum_buy_this_month)?></td><td><?=formatNumber($sum_buy_last_month)?></td><td><?=formatNumber($sum_buy_year)?></td></tr>
						<tr><td>Сумма комиссий</td><td><?=formatNumber($sum_fee_today)?></td><td><?=formatNumber($sum_fee_yesterday)?></td><td><?=formatNumber($sum_fee_this_week)?></td><td><?=formatNumber($sum_fee_this_month)?></td><td><?=formatNumber($sum_fee_last_month)?></td><td><?=formatNumber($sum_fee_year)?></td></tr>
						<tr><td>Выплачено</td><td><?=formatNumber($sum_pay_todayformatNumber)?></td><td><?=formatNumber($sum_pay_yesterday)?></td><td><?=formatNumber($sum_pay_this_week)?></td><td><?=formatNumber($sum_pay_this_month)?></td><td><?=formatNumber($sum_pay_last_month)?></td><td><?=formatNumber($sum_pay_year)?></td></tr>
					</tbody>
				</table>
				</div>
				<p class='font-weight-bold text-height-2 mt-3' >Итого:<br>
				 - начислено <span class='border_ border-secondary  p-1 rounded' ><?=formatNumber($sum_fee_all)?></span> 	<br>
				 - выплачено <span class='border_ border-secondary  p-1 rounded' ><?=formatNumber($sum_pay_all)?></span> <br>
				 - остаток к выплате <span class='border_ border-secondary p-1 rounded' ><?=formatNumber($rest_all)?></span>
				 </p>
			</div>
			<br>
			...
			<div class='mb-4' id='pay'></div>
				...
				print "<table class='table table-striped'  style='margin: 0 auto; display: inline-table;'>\n
						<thead>
						  <tr>
							<th>№</th>
							<th>Дата</th>
...
<div class="modal fade" id="cashoutModal_insales" tabindex="-1" role="dialog" aria-labelledby="cashoutModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="cashoutModalLabel_insales">Заявка на вывод средств</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="cashoutForm_insales">
                <div class="modal-body">
                    <p>Остаток к выплате: <b><?=$rest_fee?></b></p>
                    <div class="form-group">
                        <label for="amountInput_insales">Укажите сумму для вывода:</label>
                        <input type="number" name='amount' value='' class="form-control" id="amountInput_insales" placeholder="Введите сумму">
                    </div>
                    <div id="responseMessage_insales" class="alert d-none"></div> <!-- Message Placeholder -->
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Отменить</button>
                    <button type="button" id="submitCashout_insales" class="btn btn-primary">Отправить</button>
                </div>
            </form>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#submitCashout_insales').on('click', function() {
        // Get the amount input value
        var amount = $('#amountInput_insales').val();
        ...
        if (amount && amount<=<?=$rest_fee?>) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    cashout_insales: true, // Your flag to check in PHP
                    amount: amount, 
                    uid: '<?= $uid ?>' // Pass the user ID as necessary
                },
                success: function(response) {
                    $('#responseMessage_insales').removeClass('d-none alert-danger').addClass('alert alert-success').text(response); // Update the response message
                    $('#amountInput_insales').val(''); // Clear the input field
                    $('#submitCashout_insales').prop('disabled', false); // Re-enable the button
                },
                error: function() {
                    $('#responseMessage_insales').removeClass('d-none').addClass('alert alert-danger').text('Произошла ошибка.'); // Error message
                    $('#submitCashout_insales').prop('disabled', false); // Re-enable the button
                }
            });
        } else {
            // If no amount is specified, show an error
            $('#responseMessage_insales').removeClass('d-none alert-success').addClass('alert alert-danger').text('Пожалуйста, введите допустимую сумму для вывода.'); // Set the error message
            $(this).prop('disabled', false); // Re-enable the button
        }
    });
});
