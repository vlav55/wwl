...
$res = $db->query("SELECT *, msgs.tm AS tm FROM cards
                  JOIN msgs ON msgs.uid = cards.uid
                  JOIN lands ON land_num = msgs.source_id - 1000
                  WHERE utm_affiliate = '$klid' 
                  ORDER BY msgs.tm DESC");
$referralCount = $db->num_rows($res);
...
$tm1=$db->date2tm("01.01.2012"); $tm2=time();
...
if(!empty($company_logo)) {
	...
	if($width !=200 || $height !=50) {
		...
		$destinationImage = 'tg_files/logo.jpg';
}
if($ctrl_id==1)
					...
					<div class='d-flex mb-2 mt-2'>
						...
						<div class='flex-grow-1 mr-3 mt-0' style='min-width: 0;'> <!-- Crucial for flex children -->
							...
							<h5 class='mb-1' style='white-space: nowrap; overflow: hidden; text-overflow: ellipsis;'>
								<b><?=htmlspecialchars($r['name'])?> <?=htmlspecialchars($r['surname'])?></b>
							</h5>
							<p class="m-0 text-secondary small text-truncate"><?=htmlspecialchars($r['mob_search'])?></p>
							<?if(!empty(trim($r['email']))) { ?>
								<p class="m-0 text-secondary small text-truncate"><?=htmlspecialchars($r['email'])?></p>
							<?}?>
						</div>
				...
				<div class='card font_rounded p-4 border-1 shadow-sm_ mb-0 mt-md-0 mt-3'>
					<div class='d-flex justify-content-between align-items-center mb-1'>
						...
						<p class='text-primary mb-0'><?=formatNumber($sum_fee_all)?> ₽
							<a href="#" class="text-info ml-2" data-toggle="modal" data-target="#earningsDetailsModal" title="Подробнее" style="text-decoration: none;">
					...
					<div class='d-flex justify-content-between align-items-center mb-1'>
						...
						<p class='text-primary mb-0'><?=formatNumber($sum_pay_all)?> ₽
							<a href="#" class="text-info ml-2" data-toggle="modal" data-target="#paymentsDetailsModal" title="Подробнее о выплатах" style="text-decoration: none;">
								<i class="fa fa-info-circle"></i>
							</a>
						</p>
        			</div>
...
<?include "cabinet2_summary.inc.php";?>
<?include "cabinet2_earning.inc.php";?>
<?include "cabinet2_paid.inc.php";?>
<?include "cabinet2_links.inc.php";?>
<?include "cabinet2_promocodes.inc.php";?>
<?include "cabinet2_referals.inc.php";?>
