...
if(isset($_GET['del'])) {
	...
	if($tm=$db->dlookup("tm","ma_op","id='$id'")) {
		$id1=$db->fetch_assoc($db->query("SELECT id FROM ma_op WHERE del=0 AND tm<'$tm' ORDER BY tm DESC",1))['id'];
...
WHERE 
    op.del = 0
ORDER BY 
    op.tm ASC  -- Order by the 'tm' column in ascending order
...
if ($result && $result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $highlight_class = ($highlight_id && $highlight_id == $row['id']) ? 'table-warning' : '';
    ...
    print "<tr class='font-weight-bold text-black bg-warning' ><td colspan='5'></td><td>$sum_credit</td><td>$sum_debit</td><td>".$sum_credit-$sum_debit."</td colspan='3'><td></td></tr>";
