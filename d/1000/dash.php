<? 
include "/var/www/vlav/data/www/wwl/inc/dash.1.inc.php";
exit;
?>
<?
$title="DASHBOARD";
...
$cnt_new=$cp->num_rows($db->query(substr($query_new,0,$len)));

$res=$db->query("SELECT razdel.id AS razdel_id,razdel_name,COUNT(uid) AS cnt FROM cards
...
<div class='container' > 
	...
	<div class='card p-3 my-5' style='border-radius: 20px; border-color:#555;' >
		<div class='alert alert-primary' >
			Требующие внимания задачи: <a href='cp.php?view=yes&filter=new' class='' target=''><?=$cnt_new?></a>
		</div>
		  ...
		  <? foreach ($columns as $column) { ?>
			<div class="col-md-4" style="position: relative;">
				  <? foreach ($column as $row) {
						...
						//print "HERE_$s";
					  ?>
					<div class='card p-2 my-3 mx-1 bg-info_ text-white' style="<?=$s?> border-radius: 15px; border-color:#777; border-width:1px;">
