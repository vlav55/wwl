...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('sales_reports_server.php')
...
document.getElementById('filtersForm').addEventListener('submit', function(e) {
    ...
    function showRegistrationDetails(order_numbers, filters) {
        ...
        document.getElementById("dataLoader").style.display = "block";
        fetch(`sales_reports_server.php?action=getDetailsByOrder&${queryString}`)
            .then(response => response.json())
            // .then(response => response.text())
            // .then(bodyText => {console.log(bodyText); return JSON.parse(bodyText)})
            .then(data => {
                if (data && data.length) {
                    ...
                    data.forEach(registration => {
                        ...
                        if (!detailsModal.querySelector("table")) {
                            ...
                            const headers = ['Дата оплаты', 'Номер Заказа', 'Продукт', 'UID', 'Имя Фамилия', 'Сумма', 'Вид платежа'];
                        ...
                        const pay_systemCell = document.createElement('td');
                        pay_systemCell.textContent = registration.pay_system;
                        row.appendChild(pay_systemCell);

                        
                    
                        tableBody.appendChild(row);
                    });
                    // console.log("HERE_"+sum)
