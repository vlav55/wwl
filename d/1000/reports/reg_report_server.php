...
if (isset($_GET['action']) && $_GET['action'] == 'getDetailsByUid' && isset($_GET['uids'])) {
    ...
    $query = "SELECT DISTINCT msgs.*, DATE(FROM_UNIXTIME(msgs.tm)) AS registrationDate, partnerUsers.username, partnerUsers.real_user_name, cards.name, cards.surname  
      ...
      WHERE cards.del=0 AND msgs.uid IN ($uidString) AND msgs.source_id = 12
      ";
    //  AND DATE(FROM_UNIXTIME(msgs.tm)) BETWEEN '$startDate' AND '$endDate'
	...
	$query.=" ORDER BY registrationDate";
    // print_r($query);
...
elseif ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $queryByDate = "
        WITH FirstRegistrationDates AS (
            ...
            FROM msgs
            $joinClause
            WHERE cards.del=0 AND $whereClause
            GROUP BY msgs.uid
        )
        ...
        $joinClause
        WHERE cards.del=0 AND $whereClause ORDER BY registrationDate DESC
    ";
