...
if (isset($_GET['action']) && $_GET['action'] == 'getDetailsByOrder' && isset($_GET['order_numbers'])) {
    ...
    //~ $query = "SELECT DISTINCT avangard.order_number, avangard.*, DATE(FROM_UNIXTIME(avangard.tm)) AS paymentDate, cards.razdel, cards.uid, cards.id as card_id
        ...
        //~ WHERE avangard.order_number IN ($orderString) ORDER BY paymentDate";

	$query="SELECT DISTINCT 
    avangard.order_number, 
    avangard.*, 
...
FROM 
    avangard 
LEFT JOIN 
    cards ON avangard.vk_uid = cards.uid 
LEFT JOIN 
    users AS managerUsers ON cards.id = managerUsers.klid AND managerUsers.access_level = 4 
LEFT JOIN 
    users AS partnerUsers ON cards.user_id = partnerUsers.id 
LEFT JOIN 
    product ON avangard.product_id = product.id 
LEFT JOIN 
    utm ON cards.uid = utm.uid 
LEFT JOIN 
    razdel ON cards.razdel = razdel.id 
LEFT JOIN 
    tags_op ON cards.uid = tags_op.uid 
LEFT JOIN 
    tags ON tags_op.tag_id = tags.id 
WHERE 
    avangard.order_number IN ($orderString) 
    AND avangard.res = 1
";
