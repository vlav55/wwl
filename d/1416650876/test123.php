<?
// user_test.php - Run this on both working and non-working subdomains
header('Content-Type: text/plain');

echo "=== PHP User Information ===\n";
echo "Current User: " . get_current_user() . "\n";
echo "PHP SAPI: " . php_sapi_name() . "\n";
echo "Script Owner: " . getenv('USER') . "\n\n";

echo "=== File Permission Test ===\n";
$file = '/var/www/vlav/data/.env';
echo "File: $file\n";
...
echo "File Owner UID: " . fileowner($file) . "\n";

if (file_exists($file)) {
    $content = file_get_contents($file);
    echo "File read: " . ($content ? 'SUCCESS' : 'FAILED') . "\n";
} else {
    echo "FILE NOT FOUND\n";
}
...
if(isset($_GET['do_add'])) {
	...
	if(!empty($mob) && $uid=$db->dlookup("uid","cards","mob_search='$mob'") ) {
	...
	} elseif (!empty($email) && $uid=$db->dlookup("uid","cards","email='$email'") ) {
	...
	} elseif (!empty($tg_nic) && $uid=$db->dlookup("uid","cards","telegram_nic='$tg_nic'") ) {
