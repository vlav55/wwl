...
function err($arr) {
	...
	$db->notify_me("INSALES ERROR in insales_id=$insales_id\n".print_r($arr,true));
	//print_r($arr);
	return $arr;
}
...
function insales_webhook_del($webhook_id) {
    global $db,$shop, $credentials,$insales_id;
...
function insales_get_order($order_id) {
	...
	$ch = curl_init($url);
	print "$shop $credentials $passw <br>";
	print "".$url."<br>";
...
function insales_bonus_create($client_id, $amount, $descr='Бонус при регистрации') {
    global $shop, $credentials, $passw;

    // Формируем URL для создания бонуса
    $url = "https://$shop/admin/clients/$client_id/bonus_system_transactions.json";
