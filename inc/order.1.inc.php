...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if(intval($_POST['land_num'])==-1) {
		$_GET['land_num']=-1;
		$_GET['uid']=$_POST['uid'];
		$_GET['product_id']=$_POST['product_id'];
		$dt_pay_cash=$_POST['dt_pay_cash'];
		$sum_pay_cash=intval($_POST['sum_pay_cash']);
		$dateTime = DateTime::createFromFormat('d.m.Y', $dt_pay_cash);
		if($dateTime)
			$tm_pay_cash = $db->dt1($dateTime->getTimestamp());
		$comm_pay_cash=mb_substr($_POST['comm_pay_cash'],0,512);
		//$db->notify_me(print_r($_POST,true)." =$comm_pay_cash");
		//~ $db->notify_me("HERE_".date("d.m.Y H:i",$tm_pay_cash));
	}
}
...
if(isset($_GET['tg_id']) && intval($_GET['tg_id']) ) {
	$tg_id=intval($_GET['tg_id']);
	$uid=$db->dlookup("uid","cards","del=0 AND telegram_id='$tg_id'");
}
...
//~ if($db->dlookup("id","promocodes","tm2>$tm") )
		//~ $promo_hidden="";
$tm=time();
if($db->dlookup("id","promocodes","tm2>$tm AND (product_id='$product_id' || product_id=1)") && ($sum!=$base_prices[$product_id][2] || $base_prices[$product_id][2]==$base_prices[$product_id][1] ))
	$promo_hidden="";
else
	$promo_hidden="display:none;";
		...
		<div class='mt-2 p-3' style='<?=$promo_hidden?>'>
			<form class='form-inline' >
			  ...
			  <button type="submit" class="btn btn-success btn-sm ml-1" id="go" name="go" value="yes">Применить</button>
				...
				<input type="hidden" name="land_num" value=<?=$land_num?> />
				<input type="hidden" name="tm_pay_cash" value="<?=$tm_pay_cash?>" />
				<input type="hidden" name="comm_pay_cash" value="<?=$comm_pay_cash?>" />
				<input type="hidden" name="sum_pay_cash" value="<?=$sum_pay_cash?>" />
...
-->
		...
		<?if($land_num<0) {
			?>
			<button type="button" class="btn btn-primary btn-lg" id="go_cash" value='yes'>
			  Наличные
			</button>
			<button type="button" class="btn btn-primary btn-lg" id="go_bank" value='yes'>
			  Р/счет
			</button>
			<?
		} else {?>
			
			<?if(!empty($db->dlookup("prodamus_secret","pay_systems","1")) && $ctrl_id!=0) { ?>
			...
			<? } ?>
			<?if(!empty($db->dlookup("alfa_secret","pay_systems","1"))  && $ctrl_id!=1) { ?>
	...
	-->
		...
		<? } ?>
			...
			<input type="hidden" name="vk_uid" value="<?=$db->uid_md5($vk_uid)?>"/>
			<input type="hidden" name="product_id" value="<?=$product_id?>"/>
...
<script type="text/javascript">
	...
	$("#go_cash").click(function() {
		if(check_input()) {
			$('#f1').attr('action', 'pay_cash.php?land_num=-1').submit();
		}
	});
	$("#go_bank").click(function() {
		if(check_input()) {
			$('#f1').attr('action', 'pay_cash.php?land_num=-2').submit();
		}
	});
