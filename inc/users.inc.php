...
class db1 extends simple_db {
	...
	function view() {
		global $DB200;
		$res=$this->query("SELECT *,cards.uid AS uid,users.comm AS comm,users.id AS id, users.telegram_id AS telegram_id
			FROM users
			LEFT JOIN cards ON cards.id=klid
			WHERE users.del=0 ORDER BY fl_allowlogin DESC, users.id DESC");
	...
	function after_do_edit($id) {
		global $db;
		$klid=$db->dlookup("klid","users","id='$id'");
		$direct_code=$db->get_direct_code($klid);
		print "klid=$klid $direct_code"; 
		$db->query("UPDATE users SET direct_code='$direct_code' WHERE klid='$klid'");
	}
}
...
$fld=$db1->add_field("klid:","klid","","text",400); 
