<?
//imp=1 - shop visiting messages
//imp=2 - robot vklist auto answers
//imp=3 - first message auto sent by vklist_send.php
//imp=10 - event - see sorce_id
//imp=11 - chrazdel - see msgs.razdel_id
//imp=12 - note - source_id=0
...
//		if($this->vklist_mode)
	//		return true;
		...
		if($user_id) {
			...
			$this->notify_me("lock_chk error in msg.class.php 45 - user_id='$user_id'");
			return true;
		}
		$lock_tm=$r['lock_tm'];
	...
	function tbl_agent($user_id) {
		...
		<?
		if($user_id>0) {
			$real_user_name=$this->disp_name_cp($this->dlookup("real_user_name","users","id=$user_id"))." ($user_id)";
	...
	function tbl_manager() {
		...
		if(isset($_GET['ch_man_id'])) {
			...
			if($man_id) {
				...
				print "<script>location='?uid=$uid'</script>";
			}
		}
		...
		$man_name=($man_id)?$this->disp_name_cp($this->dlookup("real_user_name","users","id='$man_id'")):"НЕТ";
		if($man_id==3)
			$man_name='admin';
		...
		if($this->userdata['access_level']<=3) { //if admin
			...
			<select name='man_id' id='man_id' class='p-0 form-control' >
				<?
				$res=$this->query("SELECT * FROM users WHERE users.id>=2 AND del=0 AND fl_allowlogin=1 AND (access_level<=4) AND real_user_name!=''");
				print "<option value='0'>=не выбран=</option>";
				while($r=$this->fetch_assoc($res)) {
					$sel=($r['id']==$man_id)?"SELECTED":"";
					if($r['username']=='admin')
						$r['real_user_name']='admin';
					print "<option value='{$r['id']}' $sel>".$this->disp_name_cp($r['real_user_name'])."</option>";
				}
				?>
			</select>
...
-->
	...
	function send_avangard_email($avangard_id) {
		...
		if($_SESSION['access_level']>4) {
			print "<p class='alert alert-warning' >Нет доступа</p>";
			return;
		}

		$tmp=$this->database;
		$this->connect('vkt');
		if($this->ctrl_id==1 && !$client_ctrl_id=$this->dlookup("id","0ctrl","uid='$uid'")) {
			print "<p class='alert alert-warning' >Ошибка. У клиента нет доступа к системе ($this->ctrl_id)</p>";
			return false;
		}
		...
		if(empty($sp_template=$base_prices[$product_id]['sp_template'])) {
			print "<p class='alert alert-warning' >Не указан шаблон email для этого товара ($product_id)</p>";
			return false;
		}
		
		//$this->notify_me("$unisender_secret,$email_from,$email_from_name $sp_template");
		$vars=['uid'=>$this->uid_md5($uid),
			'passw'=>$passw,
			...
			'email'=>$client_email,
			'phone'=>$client_phone,
			'cabinet_link'=>$s->get_direct_code_link($klid),
		...
		$res=$uni->email_by_template($client_email,$sp_template,$vars);
		if(!$res)
			$this->print_r($uni->res);
		else
			print "<p class='alert alert-success' >Письмо клиенту об успешном платеже отправлено</p>";
		$uni->email_by_template("vlav@mail.ru",$sp_template,$vars);
	}
	function uid_info($uid) {
		...
		if($r['vk_id']>0) { //$uid>0
			...
			if(!$u) {
				...
				print "<div class='alert alert-warning' >Получить информацию для <b>$uid ({$r['vk_id']})</b> невозможно. </div>";
				print "<p class='alert alert-warning' >$this->error_code</p>";
				return false;
			}
			if(empty($r['name'])) {
				$r['name']=$u['first_name'];
				$this->query("UPDATE cards SET name='".$this->escape($r['name'])."' WHERE uid='$uid'");
			}
			if(empty($r['surname'])) {
				$r['surname']=$u['last_name'];
				$this->query("UPDATE cards SET surname='".$this->escape($r['surname'])."' WHERE uid='$uid'");
			}
		...
		print "<div class='card bg-light card bg-light-sm'>
			...
			if($this->is_partner_db($uid))
				$partner="<p class='alert alert-success' >🙋‍♀️ Это партнер <a href='javascript:wopen_1(\"partner.php?uid=$this->uid\")' class='' target=''><i class='	fa fa-info-circle font20' ></i></a></p>"; else	$partner="";

			$client="";
			if ($this->database=='vkt') {
				if($client_ctrl_id=$this->dlookup("id","0ctrl","uid='$uid'")) {
					$client_ctrl_dir=$this->dlookup("ctrl_dir","0ctrl","uid='$uid'");
					$client_crm="https://for16.ru/d/$client_ctrl_dir/cp.php?last10=yes";
					$client="<p class='alert alert-primary' >🔥 Создан аккаунт ВИНВИНЛАНД ($client_ctrl_id) <a  href='$client_crm' class='' target='_blank'><i class='fa fa-external-link-square'></i></a></p>";
				}
				if($cards0ctrl_id=$this->dlookup("ctrl_id","cards0ctrl","uid='$uid'")) {
					$r_cards0ctrl=$this->fetch_assoc(
									$this->query("SELECT *,cards.uid AS uid
									FROM 0ctrl
									JOIN cards ON cards.uid=0ctrl.uid
									WHERE 0ctrl.id=$cards0ctrl_id"));
					$client="<p class='alert alert-primary' >🔥 Сотрудник аккаунта $cards0ctrl_id
						{$r_cards0ctrl['company']} -
						{$r_cards0ctrl['surname']}
						{$r_cards0ctrl['name']}
						<a href='msg.php?uid={$r_cards0ctrl['uid']}' class='' target=''>перейти</a>
						</p>";
				}
			}
			...
			$uid_md5=($_SESSION['access_level']==1)?$this->uid_md5($uid)." - ".$r['uid_md5']:'';
			...
			if($r['telegram_id']) {
				if(!empty($r['telegram_nic'])) {
					$nic=$r['telegram_nic']=='vladimir_avshtolis' ? 'support' : $r['telegram_nic'];
					print "<p><a href='https://t.me/$nic' class='' title='{$r['telegram_id']}' target='_blank'><img src='/css/icons/tg-48.png'> ".$this->disp_tg($nic)."</a>\n";
				} else
					print "<p title='ник телеграм скрыт'><img src='/css/icons/tg-48.png'>\n";
				$uid_md5=$this->uid_md5($this->uid);
				print "<a href='?remove_tg=yes&uid=$uid_md5' class='' target='' title='удалить привязку телеграм для этого лида'><i class='fa fa-trash' ></i></a>";
				print "</p>";
			}
		...
		while($r=$this->fetch_assoc($res)) {
			$dt_end=($r['tm_end'])
				? "до <span class='badge bg-warning_ p-1' >".date("d.m.Y H:i",$r['tm_end'])."</span> ":"";
			$comm=!empty($r['comm']) ? "<div class='small' >".nl2br(htmlspecialchars($r['comm']))."</div>" : "";
			print "<div class='p-0 pl-2 card' ><div>
				Оплата ({$r['pay_system']}): ".date("d.m.Y H:i",$r['tm'])." ({$r['product_id']}) {$r['order_descr']}
				<span class='badge' >{$r['amount']}р.</span>
				$dt_end
				$comm";

			if($r['tm_end'])
				print "<a href='#avangard_prolong_{$r['id']}' class='' target='' data-toggle='collapse'>продлить</a>";
			if($_SESSION['access_level']<=4 && !empty($this->dlookup("sp_template","product","id='{$r['product_id']}'"))) {
				$email_for_client=" | <a href='#avangard_email_{$r['id']}' class='' target='' data-toggle='collapse' title='Отправить клиенту письмо об успешной оплате'>email</a>";
				print "$email_for_client";
			}
	...
	function do_send_wa($uid,$msg,$source_id=0,$num=0, $attach=false, $force_if_not_wa_allowed=false) {
		...
		if(!$msg=$this->prepare_msg_before_sending($uid,trim($msg))) { //for cashier loyalty card sending
		...
		if($wa->send($this,$uid,$msg,$user_id,$num,$source_id,$save_outg,$force_if_not_wa_allowed) ) {
			...
			print "<div class='alert alert-danger' >Ошибка отправки вотсап.</div>"; flush();
			sleep(5);
			exit;
			return false;
		}
	...
	function print_templates($uid,$obj) {
		...
		while($r_t=$this->fetch_assoc($res_t)) {
			$t= strpos($r_t['msg'],'{{send_loyalty_card}}')===false ? $this->prepare_msg($uid,$r_t['msg']) : $r_t['msg'];
			$t=preg_replace("/[\n\r]{2,2}/","\\n",$t);
 ...
 <div class="row">
    ...
    <div class="col-md-4">
      <div class="form-group m0" style="">
        <badge class="control-badge" for="telegram_nic">telegram: 
          <a href="https://t.me/<?=$r['telegram_nic']?>" class="" target="_blank"><?=$this->disp_tg($r['telegram_nic'])?></a>
        </badge>
        <input <?=$telegram_nic_disabled?> style="display:block;" id="telegram_nic" class="form-control" type="text" name="telegram_nic" value="<?=htmlspecialchars($this->disp_tg($r['telegram_nic']))?>">
      </div>
    </div>
  </div>
	...
	function delay_ctrl($pass) {
		...
		$planned= $this->tbl_delayed(['tm_delay'=>$tm_delay,'tm_delay_imp'=>$tm_delay_imp]);
		preg_match("/\bbg-[a-zA-Z-]+\b/",$planned,$m);
		$bg_planned=$m[0];
		$text_planned=!$tm_delay ? "text-black" : "text-white";
		?>
		<div>
<!--
...
-->
			...
			<input type='text' id='__dt_delay_<?=$pass?>' name='dt_delay' value='<?=$dt_delay?>' class='form-control text-center p-1 <?=$text_planned?> <?=$bg_planned?>'  style='display:inline_;width:140px;'>
			<input type='time' id='__hi_delay' name='hi_delay' value='<?=$hi_delay?>' class='form-control text-center p-0 <?=$text_planned?> <?=$bg_planned?>' data-input style='display:inline_;width:120px;'>
...
-->
			...
			//	print "&nbsp;&nbsp;<input type='submit' class='btn btn-success' name='mark_delayed' value='ОТЛОЖИТЬ' $disabled>\n";
				...
				$wa_disabled=(!empty($this->pact_secret) && intval($this->pact_company_id))?"":"disabled";
				//$wa_disabled="disabled";
				$insta_disabled=($r['pact_insta_cid'])?"":"disabled";
				$tg_disabled=($r['telegram_id'])?"":"disabled";
				$gpt_icon=$r['fl_gpt'] ? "<a href='?gpt_off=yes&uid=$uid#send_form' class='' target='' title='ИИ включен. На входящие будет отвечать ИИ. Нажмите, чтобы отключить'>⚛️</a>" : "";
				//print "HERE_".$r['email'];
...
-->
					<button id="assign-tag-btn" style="" class='btn btn-primary btn-sm' >
						<i class='fa fa-plus text-white' ></i> тэг
					</button>
...
$(document).ready(function(){
  $('[data-toggle="tooltip"]').tooltip();
	...
	function top_info() {}
	function page_reload($alert) {
		$_SESSION['alert']=$alert;
		print "<script>location='?uid=$this->uid&alert=yes'</script>";
	}
	function run() {
		global $msg_pay_info,$tg_bot_msg_name;
		...
		$uid=$this->get_uid($_GET['uid']);
		if(!$uid) {
			//print "<div class='alert alert-danger' >Error uid=0</div>
			print "<script>location='cp.php?view=yes'</script>";
			header("Location: cp.php?view=yes");
			exit;
		}
		...
		if(isset($_GET['remove_tg'])) {
			print "<p class='alert alert-warning' >
				Удалить привязку телеграм?
				<a href='?uid=$uid&do_remove_tg=yes' class='' target='_blank'>Удалить</a>
				<a href='?uid=$uid' class='' target='_blank'>Отменить</a>
				</p>";
		}
		if(isset($_GET['do_remove_tg'])) {
			$this->query("UPDATE cards SET telegram_id=0,telegram_nic='' WHERE uid='$uid'");
			if($klid=$this->dlookup("id","cards","uid='$uid'"))
				$this->query("UPDATE users SET telegram_id=0 WHERE klid=$klid");
			//$alert= urlencode("<p class='alert alert-warning' >Привязка к телеграм удалена. Подключить телеграм бот и восстановить привязку лид может <a href='https://t.me/$tg_bot_msg_name?start=$uid_md5' class='' target='_blank'>по ссылке</a> </p>");
			print "<script>location='?uid=$uid'</script>";
		}
		...
		if(isset($_GET['goto_cp'])) {
			...
			$klid=$this->dlookup("id","cards","uid='$uid'");
			print "<script>location='cp.php?view=yes&uid=$uid&follow=yes#r_$klid'</script>";
		}
		
		print "<div><a href='?goto_cp=yes' class='btn btn-primary btn-lg mr-3' target=''><i class='fa fa-sign-out far-2x' aria-hidden='true'></i></a>
		...
		if(!$p->is_access_allowed($_SESSION['userid_sess'],$uid)) {
			$this->page_reload( "<div class='alert alert-danger' >Ошибка. Этот клиент ($uid) не назначен для ({$_SESSION['userid_sess']}).</div>");
			exit;
		}
		...
		if(!$this->vklist_mode) {
			...
			if(!$r) {
				$this->page_reload( "<p class='alert alert-danger'>Ссылка недействительна: $uid</p>");
				exit;
			}
	...
	$acc_id=$this->dlookup("id","vklist_acc","del=0 AND fl_allow_read_from_all=1");
		...
		if(isset($_GET['dt_hi_delay_set'])) {
			if($tm=$this->date2tm($_GET['dt_delay'])) {
				if($hi=$this->time2tm($_GET['hi_delay'])) {
					...
					//$this->lock_off($uid);
					$this->page_reload( "<div class='alert alert-warning' >Перенесен на <span class='bg-primary text-white p-2' >".date("d.m.Y H:i",$tm)."</span></div>");
				}
			}
		}
		if(isset($_GET['to_0min'])) {
			...
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Оставлен в задачах</div>");
		...
		if(isset($_GET['to_15min'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на 15 мин (".date("H:i",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_1hour'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на 1 час (".date("H:i",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_3hour'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на 3 часа (".date("H:i",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_tomorrow'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на завтра (".date("d.m.Y",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_2days'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на 2 дня (".date("d.m.Y",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_1week'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на неделю (".date("d.m.Y",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['to_month'])) {
			...
			$this->mark_new($uid,$fl=0);
			//$this->lock_off($uid);
			$this->page_reload( "<div class='alert alert-warning' >Перенесен на 30 дней (".date("d.m.Y",$tm).")</div>");
			//~ print "<a href='javascript:window.close()' class='' target=''><button class='btn btn-primary' >Закрыть</button></a>";
			//~ print "<script>opener.location.reload();</script>";
			//~ exit;
		}
		if(isset($_GET['clr_delay'])) {
			...
			$this->mark_new($uid,$fl=0);
			$this->page_reload("<div class='alert alert-warning' >Отслеживание прекращено! (увидим автоматически, если проявит активность)</div>");
		}
		...
		if(isset($_POST['do_send_msg'])) {
			...
			$this->page_reload("<p class='alert alert-info' >Сообщение отправлено в ВК</p>");
		}
		...
		if(isset($_POST['do_send_wa'])) {
			...
			$this->page_reload("<p class='alert alert-info' >Сообщение отправлено в WA</p>");
		}
		if(isset($_POST['do_send_tg'])) {
			$this->do_send_tg($uid,$_POST['msg']);
			$this->page_reload("<p class='alert alert-info' >Сообщение отправлено в ТГ</p>");
		}
		...
		if(isset($_POST['do_send_insta'])) {
			...
			$this->page_reload("<p class='alert alert-info' >Сообщение отправлено в INSTAGRAM</p>");
		}
		...
		$this->test_microtime(__LINE__);
			...
			//$this->print_r($res);
				...
				//print_r($res);
				if(isset($res['response'])) {
					$emails=$this->prepare_correspondence_from_db("SELECT *,msgs.id AS id, msgs.uid AS uid,msgs.acc_id AS acc_id FROM msgs LEFT JOIN users ON users.id=user_id WHERE msgs.uid=$uid AND (msgs.acc_id=10 OR msgs.acc_id=2 OR msgs.acc_id=100 OR msgs.acc_id=101 OR msgs.acc_id=102 OR msgs.acc_id=103)");
					//print_r($emails);
					$msgs_events=$this->prepare_correspondence_from_db("SELECT *,msgs.id AS id,msgs.uid AS uid,msgs.acc_id AS acc_id FROM msgs LEFT JOIN users ON users.id=user_id WHERE msgs.uid=$uid AND outg>1");
...
//							if($_SESSION['username']=='vlav') { $this->print_r($r); }
	...
	function prepare_correspondence_from_db($query) {
		...
		while($r=$this->fetch_assoc($res)) {
			...
			if(strpos($r['msg'],"call_id")!==false) {
				...
				$msgs[$r['tm']]=array('id'=>$r['id'],'uid'=>0,'txt'=>($r['msg']),'outg'=>$r['outg'],'read'=>1,'acc_id'=>$r['acc_id'],'user_id'=>htmlspecialchars($r['username']),'user_id_ok'=>$r['user_id'], 'source_id'=>$r['source_id']);
			} else {
				//print "HERE111 {$r['id']} {$r['uid']} <br>";
				$msgs[$r['tm']]=array('id'=>$r['id'],'uid'=>0,'txt'=>(htmlspecialchars($r['msg'])),'outg'=>$r['outg'],'read'=>1,'acc_id'=>$r['acc_id'],'user_id'=>htmlspecialchars($r['username']),'user_id_ok'=>$r['user_id'],'source_id'=>$r['source_id']);
			}
	...
	//	$this->print_r($msgs);
		...
		$uid=$this->uid;
		$res=$this->query("SELECT *,msgs.id AS id, msgs.uid AS uid,msgs.acc_id AS acc_id FROM msgs JOIN users ON users.id=user_id WHERE msgs.uid=$uid AND imp=1",0);
		while($r=$this->fetch_assoc($res)) {
			$read=isset($r['read'])?$r['read']:1;
			$msgs[$r['tm']]=array('id'=>$r['id'],'txt'=>htmlspecialchars($r['msg']),'outg'=>$r['outg'],'read'=>$read,'acc_id'=>$r['acc_id'],'user_id'=>htmlspecialchars($r['username']),'user_id_ok'=>$r['user_id']);
		}
