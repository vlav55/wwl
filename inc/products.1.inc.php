...
<div>
	<a href='javascript:window.close()' class='btn btn-warning'>Закрыть</a>
	<a href='javascript:history.back()' class='btn btn-link'>вернуться</a>
</div>
<div class="container">
    <div class="row">
        <div class="col">
...
switch($action) {
    ...
    case 'copy':
        ...
        if ($action == 'update' || $action == 'copy' ) {
            $stmt = $db->query("SELECT * FROM `product` WHERE del=0 AND `id` = '$productId'");
        ...
        if (!empty($_POST)) {
            ...
            $sku = $db->escape(htmlentities(mb_substr(trim($_POST['sku']),0,2048)));
            $descr = $db->escape(htmlentities(mb_substr(trim($_POST['descr']), 0, 1024)));
            $jc = $db->escape(htmlentities(mb_substr(trim($_POST['jc']), 0, 32)));
            ...
            $spTemplate = $db->escape(htmlentities(mb_substr(trim($_POST['sp_template']), 0, 64))) ?? null;

            if ($db->dlookup("id", "product", "del=0 AND id='$id' AND id!='$productId'") || $id<=0) {
                $errorMessage = '<div class="alert alert-warning">Продукт с ID: ' . $id . ' уже существует или недопустимое значение.</div>';
            } else {
        ...
        <form action="#" method="POST">
            <div class="form-group">
                <label for="__id">НОМЕР</label>
                <input type='number' <?php echo $id_disabled; ?> name="id" class="form-control" id="__id" value="<?php echo $product["id"]; ?>">
            </div>
            <div class="form-group">
                <label for="sku">SKU</label>
                <input type='text' name="sku" class="form-control" id="sku" value="<?php echo $product["sku"]; ?>">
            </div>
            <div class="form-group">
                <label for="descr">Наименование</label>
                <input type='text' name="descr" class="form-control" id="descr" value="<?=$product["descr"]?>">
            </div>
            <div class="form-group">
                <label for="__price0">Цена зачеркнутая</label>
                <input type='number' name="price0" class="form-control" id="__price0" value="<?php echo $product["price0"]; ?>">
            </div>
            <div class="form-group">
                <label for="__price1">Цена базовая</label>
                <input type='number' name="price1" class="form-control" id="__price1" value="<?php echo $product["price1"]; ?>">
            </div>
            <div class="form-group">
                <label for="__price2">Цена со скидкой</label>
                <input type='number' name="price2" class="form-control" id="__price2" value="<?php echo $product["price2"]; ?>">
            </div>
            <div class="form-group">
                <label for="__term">Срок доступа, дн</label>
                <input type='number' name="term" class="form-control" id="__term" value="<?php echo $product["term"]; ?>">
            </div>
            <div class="form-group">
                <label for="__fee_1">Партнерское вознаграждение уровень 1, % или фикс</label>
                <input type='number' name="fee_1" class="form-control" id="__fee_1" value="<?php echo $product["fee_1"]; ?>">
            </div>
            <div class="form-group">
                <label for="__fee_2">Партнерское вознаграждение уровень 2, % или фикс</label>
                <input type='number' name="fee_2" class="form-control" id="__fee_2" value="<?php echo $product["fee_2"]; ?>">
            </div>
            <div class="form-group">
                <label for="__fee_cnt">На сколько продаж начислять вознаграждение (0 без ограничений)</label>
                <input type='number' name="fee_cnt" class="form-control" id="__fee_cnt" value="<?php echo $product["fee_cnt"]; ?>">
            </div>
            <div class="form-group">
                <label for="__sp_template">Шаблон email при успешной оплате</label>
                <input type='text' name="sp_template" class="form-control" id="__sp_template" value="<?php echo $product["sp_template"]; ?>">
            </div>
            <div class="form-group">
    ...
    case 'del':
		$db->chk_column('product', 'fl_cashier', 'tinyint', true);
		if($db->dlookup("id","product","id = '$productId' AND fl_cashier=0")) {
			$db->query("UPDATE `product` SET del = 1 WHERE `id` = '$productId' AND fl_cashier=0");
			header("Location: ?");
			exit;
		} else {
			print "<script> alert('Продукт используется системой Лояльность 2.0, удаление невозможно');
    ...
    default:
        $stmt = $db->query("SELECT * FROM `product` WHERE del = 0 AND id>0 ORDER BY id ASC");
            ...
            echo "<tr id='item{$product["id"]}'>
                    <td>
                        ...
                        <div class='btn-group'>
                            ...
                            &nbsp;
                            <a class='font20' href='?action=copy&id={$product["id"]}'><i class='fa fa-copy'></i></a>
                            &nbsp;
